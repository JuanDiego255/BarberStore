...
class FrontendController extends Controller
{
    ...
    {
        $templateSection = ['hero', 'about-area', 'open-shop', 'experience', 'speciality', 'process-behind', 'team',  'book-appointment', 'why-chose-us', 'services', 'gallery', 'plan', 'testimonial', 'blog', 'faq',];
    ...
    {
        $data['title'] = "Blog";
        $data['blogs'] = Blog::with('blogDetails')->latest()->paginate(3);
        $data['blogCategory'] = BlogCategory::with('blogCategoryDetails')->withCount('blog')->latest()->get();
    ...
    public function categoryWiseBlog($slug = null, $id)
    {
        ...
        $data['blogs'] = Blog::with(['blogDetails', 'category'])->where('blog_category_id', $id)->latest()->paginate(3);
        $data['blogCategory'] = BlogCategory::with('blogCategoryDetails')->withCount('blog')->latest()->get();
        return view($this->theme . 'blog', $data);
    }
    ...
    {
        $data['blogDetails'] = Blog::with('blogDetails')->findOrFail($id);
        $blogCategory = $data['blogDetails']->blog_category_id;


        $data['recentBlog'] = Blog::with('blogDetails', 'category')
            ->where('blog_category_id', $blogCategory)
            ->where('id', '!=', $id)
            ->latest()
            ->get();
    ...
    public function blogSearch(Request $request)
    {
        ...
        $data['blogCategory'] = BlogCategory::with('blogCategoryDetails')->withCount('blog')->latest()->get();
        $data['title'] = "Blog";
        $data['blogs'] = Blog::with('blogDetails', 'category')
            ->whereHas('category.blogCategoryDetails', function ($qq) use ($search) {
                $qq->whereRaw("name REGEXP '[[:<:]]{$search}[[:>:]]'");
            })
            ->orWhereHas('blogDetails', function ($qq2) use ($search) {
                $qq2->whereRaw("title REGEXP '[[:<:]]{$search}[[:>:]]' or description REGEXP '[[:<:]]{$search}[[:>:]]'");
            })
            ->latest()->paginate(3);
    ...
    public function categoryWiseBlogSearch(Request $request)
    {
        $search = $request->search;
